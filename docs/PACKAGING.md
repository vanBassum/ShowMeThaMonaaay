...
Status: **partly built.** `tools/pack_model.py` (assembler) and `app/backend/models.py`
(download-from-release into AppData) exist; the rest is in "Build order" at the bottom.
...
— anything reported during/after has a later timestamp and rolls to the next model. Reasoned
*within an `icons_fingerprint` lineage* (icon-id-based fixes only apply to a matching vocab).

## Model cards & the root index

`manifest.json` is machine-readable; the **cards** are the human story. Two levels: one card
*per model* (shipped in its archive), and one **root index** for the whole model program.

### Per-model card — `MODEL_CARD.md` (shipped in each archive)

`tools/pack_model.py` bundles it; the existing `<training>/models/archive/<name>/MODEL_CARD.txt`
convention is the seed — formalise it to Markdown. Sections:

- **Identity** — name, version, parent, `built_at`, `classes`, `icons_fingerprint`.
- **What changed vs. predecessor** — the key delta line (augs added, vocab change, fixes).
  E.g. *"v3 = v2 + 90° rotation + colored-cell-bg aug; rotated items 0/24 → 24/24."*
- **Training** — dataset (synthetic gen + the **cumulative** augmentation list), warm-start
  parent, epochs / imgsz / patience, hardware, runtime, where it ran (local / Kaggle).
- **Results** — mAP50 + real-stash detection counts; honest about regressions.
- **Class vocabulary** — count, ambiguous (exact-dup) handling, dedupe collapse if any.
- **Reports included** — `reports_cutoff` + the report/fix batch IDs folded in (auditable).

### Root index — the master record

What `docs/MODELS.md` becomes (human root card) plus a machine-readable twin: every model
line + version, parent lineage, each version's `icons_fingerprint` + release tag, the
reports-inclusion timeline, and the shared link-DB / catalog-source context. Publish a
**`models-index.json`** (own release tag) so the app's model picker can discover available
models, lineage, and compatibility without downloading each archive.
...
1. **Promote `icon_item_map.json`** out of gitignored `data/` into a shippable/built
   location — visual-matcher output, not packageable as-is. *(blocker; still pending)*
2. **Emit `icon_hashes.json`** from `build_dataset.py` (reuse `icon_dups.sig`), atomic with
   the trained class set. *(pending — pack_model.py currently retrofits it from the cache)*
3. ✅ **Assembler `tools/pack_model.py`** — collects model + link map + hashes, writes
   `manifest.json` (sha256s + fingerprint), zips it. *(built; extend per #7–8)*
4. **CI release workflow** — on a model tag, run the assembler and attach the zip to a release.
5. **First-run downloader** — ✅ `app/backend/models.py` fetches by release tag → AppData +
   reads the manifest. *Pending:* sha256 verification, `min_app_version`/`classes` compat
   check, and the catalog/icon fetch-on-demand path.
6. **Drift check** (later) — re-hash live cache vs `icon_hashes.json`; feed report-to-server.
7. **Bundle `MODEL_CARD.md`** in the archive (formalise the `MODEL_CARD.txt` convention) and
   add `parent` + `reports_cutoff` to `manifest.json`. `pack_model.py` emits both.
8. **Root index** — make `docs/MODELS.md` the human root card + generate `models-index.json`
   (own release tag) for the app's model picker / lineage discovery.
